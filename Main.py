...
def main():
    ...
    print("Welcome to Artemis, the greatest village within the country of Ty'rex! Well, maybe not the greatest but a decent village none the less!")

    # Player Name Input
    player_name = input("It's not often that we meet travelers out here. I dont mean to be rude, but what was your name again?")
    player_hp = 30
    companion_name = 'Vax'
    companion_hp = 40
    companion_healing = 3
    ...
    # Choosing Weapon
    first_player_weapon_choice = int(input("I'm not sure how you made it this long traveling through the dark forest without a weapon. I have a spare iron sword, mace, and spear. (type a number. 1 = sword, 2 = mace, and 3 = spear)"))
    ...
    if (first_player_weapon_choice == 1):
        ...
        print("Wonderful choice! That sword is a versatile weapon that hits for a strong 6 slashing damage. Take a shield as well.")

    elif (first_player_weapon_choice == 2):
        first_player_weapon = iron_mace
        first_player_weapon_dmg_type = 'bludgeoning'
        print("Wonderful choice! That mace is a blunt weapon that hits for a hard 5 bludgeoning damage. Take a shield as well.")

    elif (first_player_weapon_choice == 3):
        first_player_weapon = iron_spear
        first_player_weapon_dmg_type = 'piercing'
        print("Wonderful choice! That spear is a strong weapon that hits for a strong 7 piercing damage. Take a shield as well.")
    ...
    print("We have a grave yard in the forest behind the inn that is frequented by skeletons, ghosts, and ghouls. Lets go to check it out.", end=' ')
    print("After a 15 minute walk the graveyard is coming into view. Cracking twigs and rustling bushes alert both and Vax of an extremely close skeleton that went previously unnoticed.")
    print("(Players only have 30 Health Points so play carefully)")

    skeleton_hp = 15
    skeleton_dmg = 3
    attack_on_skeleton = int(input("(In combat there will be options for the player to choose. 1 = weapon attack, 2 = unarmed attack, 3 = shield(Block all incoming damage))(Vax can grapple which will make the player do double damage))"))
    ...
    if (attack_on_skeleton == 1):
        ...
        if (first_player_weapon == iron_spear):
            ...
            print("The remaining HP for the skeleton is ", skeleton_hp, ". \nSpears normally do 7 damage but are piercing damage which is half damage against skeletons and some other undead.", sep="")
        ...
        # If program never makes it to the else... so swords are hitting like a mace
            ...
            # Multiplication *
            skeleton_hp = skeleton_hp - first_player_weapon * 2
            print("The remaining HP for the skeleton is ", skeleton_hp, " health. Maces normally do 6 damage but are bludgeoning damage which is doubled damage against skeletons and some other undead.", sep="")
            player_hp = player_hp - skeleton_dmg
            print("The player is at ", player_hp, ".", sep="")

        elif (first_player_weapon == iron_sword):
            skeleton_hp = skeleton_hp - first_player_weapon
            print("The remaining HP for the skeleton is ", skeleton_hp, ". Slashing damage is neutral against skeletons so the damage done is 6.", sep="")
            player_hp = player_hp - skeleton_dmg
            print("The player is at ", player_hp, ".", sep="")
    ...
    while (skeleton_hp > 0):
        ...
        if (player_hp <= 0):
            ...
            attack_on_skeleton = int(input("The skeleton is now grappled which will result in double damage. What do you choose to do?(Same 1-3 a s above)"))
    ...
    def player_died():
        if (player_hp <= 0):
            replay = input("The player has died fighting the good fight. Would you like to restart as a new champion?(Type: yes or no)")
    ...
    # -----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    print("Hell yeah! Nice fighting there! I'd love to celebrate but we made a huge commotion. We should probably get out of here before... there's a ghoul heading right towards us.")
    ...
    enemy_fight_no_second(ghoul_hp, player_hp, first_player_weapon, ghoul_dmg, companion_hp, ghoul_name, companion_healing, iron_spear, iron_mace, enemy_status, companion_first_weapon, fist)

#############################################################################################################################################################################

#neutral fight because the enemy has no weaknesses or strengths
def enemy_fight_no_second(enemy_hp, player_hp, first_player_weapon, enemy_dmg, companion_hp, enemy_name, companion_healing, iron_spear, iron_mace, enemy_status, companion_first_weapon, unarmed):
    ...
    while (enemy_hp > 0):
        ...
        if (player_hp <= 0):
            ...
            if companion_hp <= 0:
                print("Vax has passed out and can no longer assist in battle. Finish the fight quick so you can heal him")
                who_attacks = 1

            else:
                who_attacks = int(input("From now you'll command both Vax and yourself in battle to allow more flexibility in combat(Type: 1 to move yourself first or 2 to move Vax first)"))
                ...
                if who_attacks == 1:
                    attack_on_enemy = int(input("You're moving first. 1 = weapon attack, 2 = unarmed attack, 3 = shield(Block all incoming damage)"))
                    ...
                    if (attack_on_enemy == 1):
                        ...
                        if (first_player_weapon == iron_spear):
                            ...
                            if (enemy_status == 'grappled'):
                                print("The ", enemy_name, " is currently grappled, but will be released now that you've attacked", sep='')
                    ...
                    attack_on_enemy_companion = int(input("Vax moves next. 1 = weapon attack(Vax has a sword), 2 = healing, 3 = shield(Block all incoming damage), 4 = grapple(lasts only one turn"))
                    ...
                    if (attack_on_enemy_companion == 1):
                        ...
                        healing_choice = int(input("who are you healing?(Type 1 to heal you and 2 to have Vax heal himself)"))
                ...
                # If Vax moves first
                if who_attacks == 2:
                    attack_on_enemy_companion = int(input("Vax moves first. 1 = weapon attack(Vax has a sword), 2 = healing, 3 = shield(Block all incoming damage), 4 = grapple(lasts only one turn"))
                    ...
                    attack_on_enemy = int(input("You're moving second. 1 = weapon attack, 2 = unarmed attack, 3 = shield(Block all incoming damage)"))
    ...
    # --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
    print("Well I can see now that you have plenty of combat experience. I've never seen someone kill two undead with such ease!")
    print("This ghoul must have been an seasoned adventurer judging by the weapons on his belt. He has a spell book and a magic dagger. I'll take one and you can take the other.")
    ...
    while (second_player_weapon_choice != 'book'):
        attack_on_enemy = input("Which do you want? The dagger or the spell book? (type: 1 for the book or 2 for the dagger)")
    ...
    if (second_player_weapon_choice == 'book'):
        ...
        print("The book has a blue cover with a lighting bolt streaking across it. Opening the book floods you with the knowledge of how to use a basic lightning spell. (10 lighting damage)")
    elif (second_player_weapon_choice == 'dagger'):
        second_player_weapon = magic_dagger
        # second_player_weapon_dmg_type == 'poison'
        print("The dagger has a green threaded handle and a quarter sized circle in the center of the blade. Touching it granted the knowledge of how to use this magic weapon. (9 poison damage)")
        print("Cutting any living creature with it will instantly hurt the creature with terrible poison damage. This weapon will have no effect on undead")
    else:
        second_player_weapon = lightning_spell
        # second_player_weapon_dmg_type == 'lighting'
        print("(You get the spell then) The book has a blue cover with a lighting bolt streaking across it. Opening the book floods you with the knowledge of how to use a basic lightning spell. (10 lighting damage)")
